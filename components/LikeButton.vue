<template>
  <div class="d-inline-block">
    ...
    <!-- Like -->
    <b-button v-if="!hasVoted && !showLikes" :size="size" variant="primary" class="like-button" @click="$refs['voteControlModal-' + permlink].show()">
    ...
    <!-- Unlike -->
    <b-button v-if="hasVoted && !showLikes" :size="size" variant="outline-secondary" class="like-button" @click="vote(0)">
      <svg v-if="!loading" viewBox="0 0 24 24">
    ...
    <!-- Like -->
    <b-button-group v-if="!hasVoted && showLikes" :size="size" class="like-button-group">
    ...
    <b-modal :id="'voteControlModal-' + permlink" :ref="'voteControlModal-' + permlink" hide-footer :title="$t('votemodal.title')">
      <div class="text-center">
        <small class="text-muted">{{ $t('votemodal.adjuststrength') }}</small>
      </div>
      <ul class=" vote-control pagination justify-content-center mt-2">
        <li :class="{'page-item': true, disabled: voteWeight === 0}">
          <a href="#" class="page-link vote-controls" @click.prevent="setVoteWeight(voteWeight - 10)">-10</a>
        </li>
        <li :class="{'page-item': true, disabled: voteWeight === 0}">
          <a href="#" class="page-link vote-controls" @click.prevent="setVoteWeight(voteWeight - 1)">-1</a>
        </li>
        <li class="page-item vote-weight">
          <input v-model="voteWeight" type="number" class="form-control" min="0" max="100">
        </li>
        <li :class="{'page-item': true, disabled: voteWeight === 100}">
          <a href="#" class="page-link vote-controls" @click.prevent="setVoteWeight(voteWeight + 1)">+1</a>
        </li>
        <li :class="{'page-item': true, disabled: voteWeight === 100}">
          <a href="#" class="page-link vote-controls" @click.prevent="setVoteWeight(voteWeight + 10)">+10</a>
      ...
      <div class="text-center">
        <button type="submit" :class="'btn btn-' + (voteWeight > 0 ? 'primary' : 'danger' )" @click="vote(voteWeight)">
          <i :class="'fas fa-' + (voteWeight > 0 ? 'thumbs-up' : 'ban' )" /> {{ voteWeight > 0 ? $t('likebutton.like') : $t('likebutton.unlike') }}
        </button>
      </div>
    </b-modal>
  </div>
